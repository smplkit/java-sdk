...
class ApiExceptionHandlerTest {
    ...
    void deriveMessage_nullList_returnsDefault() {
        assertEquals("An API error occurred", ApiExceptionHandler.deriveMessage(null));
    }

    @Test
    void deriveMessage_emptyList_returnsDefault() {
        assertEquals("An API error occurred", ApiExceptionHandler.deriveMessage(List.of()));
    }
    ...
    @Test
    void networkException_causeWithNullMessage_fallsBackToCauseToString() {
        // Cause present but with a null message -> the ternary on the cause-message
        // branch must fall back to cause.toString().
        var causeWithNoMessage = new IllegalStateException();
        var wrapper = new RuntimeException((String) null, causeWithNoMessage);

        SmplError ex = ApiExceptionHandler.mapApiException(wrapper);

        assertInstanceOf(ConnectionError.class, ex);
        assertTrue(ex.getMessage().contains("IllegalStateException"),
                "Expected cause.toString() fallback but got: " + ex.getMessage());
    }

    @Test
    void networkException_noCauseAndNullMessage_fallsBackToExceptionToString() {
        // No cause and a null message -> the no-cause ternary must fall back to
        // e.toString().
        var wrapper = new RuntimeException();

        SmplError ex = ApiExceptionHandler.mapApiException(wrapper);

        assertInstanceOf(ConnectionError.class, ex);
        assertTrue(ex.getMessage().contains("RuntimeException"),
                "Expected e.toString() fallback but got: " + ex.getMessage());
    }
}
